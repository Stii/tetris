...
impl Tetrimino {
    ...
    fn test_position(&self, game_map: &[Vec<u8>],
                     temp_state: usize, x: isize, y: usize) -> bool {
        for decal_y in 0..4 {
            for decal_x in 0..4 {
                let x = x + decal_x;
                if self.states[tmp_states][decal_y][decal_x as usize] != 0
                    &&
...
fn handle_events(tetris: &mut Tetris, quit: &mut bool, timer: &mut SystemTimer, event_pump: &mut sdl2::EventPump) -> bool {
...
fn main() {
    ...
    let window = video_subsystem.window("rust_sdl2 demo: Video", 800, 600)
        .position_centered()
        .opengl()
        .build()
    ...
    let green_square = create_texture_rect(&mut canvas,
                                           &texture_creator,
                                           TextureColor::Green,
                                           TEXTURE_SIZE).expect("Failed to create texture: Green");
    let blue_square = create_texture_rect(&mut canvas,
                                          &texture_creator,
                                          TextureColor::Blue,
                                          TEXTURE_SIZE).expect("Failed to create texture: Blue");
